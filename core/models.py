...
class TestCase(BaseModel):
    ...
    用例名称: str = Field(description="测试用例的简洁标题")
    优先级: str = Field(description="用例的优先级 (例如：高, 中, 低)")
    用例类型: str = Field(description="用例的类型 (例如：功能测试, 性能测试)", default="功能测试")
    前置条件: str = Field(description="执行测试前需要满足的前置条件")
    步骤描述: str = Field(description="执行测试的具体步骤描述")
    预期结果: str = Field(description="预期的测试结果")
